...
파트 A, B, C, D 발표를 각각 하나의 흐름으로 설명하기 위한 화이트보드용 문서입니다.
질문 목록을 나열하는 대신, 어떤 그림을 먼저 그리고 어떤 문장으로 다음 장면으로 넘어갈지 기준으로 정리했습니다.
각 Scene 뒤에는 `### 직접 검증` 블록이 붙어 있어, strace / tcpdump / ss / /proc 로 그 장면을 실시간으로 확인하면서 발표할 수 있습니다.
...
- [part-d-whiteboard-topdown.md — Part D OS 기반 (FD · 프로세스 · 가상 메모리) 발표안](./part-d-whiteboard-topdown.md)
...
| 99 | 화이트보드 세션 설계 — curl 한 줄을 바닥까지 그려서 설명하기 (90분 플로우) | [99-whiteboard-session.md](./99-whiteboard-session.md) |
| PA | Part A 화이트보드 발표안 — write 에서 wire 까지 (검증 포함) | [part-a-whiteboard-topdown.md](./part-a-whiteboard-topdown.md) |
| PB | Part B 화이트보드 발표안 — 주소, DNS, handshake (검증 포함) | [part-b-whiteboard-topdown.md](./part-b-whiteboard-topdown.md) |
| PC | Part C 화이트보드 발표안 — Tiny, CGI, Proxy, 동시성 (검증 포함) | [part-c-whiteboard-topdown.md](./part-c-whiteboard-topdown.md) |
| PD | Part D 화이트보드 발표안 — FD · 프로세스 · 가상 메모리 (검증 포함) | [part-d-whiteboard-topdown.md](./part-d-whiteboard-topdown.md) |
...
<<<<<<< HEAD
| 커널 코드 레벨 올인원                | q04 q10 q18 q19             |
| 프로세스 조상 / daemonize            | q13 q16 q19                 |
| 칠판 드로잉 플로우 (발표용)          | 99 -> 00 -> q01..q19        |
=======
