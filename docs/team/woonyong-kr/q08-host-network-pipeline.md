...
#### 왜 라우터의 MAC 이 바뀌는가 — 인터페이스마다 MAC 이 따로 있다

처음 보면 "방금 들어올 땐 라우터 MAC이 `11:11:..` 였는데, 나갈 땐 왜 `22:22:..` 로 달라지나?" 가 헷갈린다. 핵심은 하나다.

> **라우터는 MAC 주소가 1개가 아니라, 꽂혀 있는 인터페이스(포트)마다 1개씩**을 가진다.

일반 PC NIC 도 LAN 이 두 개면 포트가 두 개고 MAC 도 두 개다. 라우터는 그 구조가 본업일 뿐이다.
...
프레임이 라우터를 통과할 때 벌어지는 일을 풀면:

```
[클라이언트 A 에서 출발]
 프레임 ①
   dst MAC = 11:11:11:11:11:11  (라우터 R 의 eth0, LAN1 쪽)
   src MAC = AA:AA:AA:AA:AA:AA  (클라이언트 A)
   IP src/dst = A -> B
   TTL = 64
        │
        v LAN1 선로
...
[라우터 R 의 eth0 수신]
  ...
  IP 계층  : "IP dst 가 내 IP 아님 -> 포워딩"
             라우팅 테이블: B 는 LAN2 방향
             ARP 캐시   : B 의 MAC = BB:BB:BB:BB:BB:BB
...
[서버 B 수신]
 프레임 ②
   ...
   src MAC = 22:22:22:22:22:22  <- R 의 LAN2 쪽!
   IP src/dst = A -> B (그대로)
```

**왜 이렇게 설계됐나** — 이더넷(MAC) 은 "같은 LAN 안의 한 구간" 만 담당하는 주소다. 서로 다른 LAN 은 MAC 만으로는 서로 도달할 수 없다. 스위치는 자기 LAN 에 붙은 MAC 만 기억하지 다른 LAN 의 MAC 은 알 수가 없기 때문이다.

그래서 규칙이 이렇게 정해진다:

| 구간 | 바뀌나? | 이유 |
| --- | --- | --- |
| **IP src/dst** | **안 바뀜** | "누가 누구에게" 라는 논리 주소. end-to-end |
| **MAC src/dst** | **매 홉마다 교체** | "현재 구간의 두 이웃" 이라는 물리 주소. hop-by-hop |
| TTL | 매 홉마다 −1 | 루프 방지 |
...
"보낸 쪽이 R 이니까 `11:11:11:11:11:11` 아닌가?" — `11:11:..` 은 **R 의 LAN1 쪽 포트(eth0) MAC** 이다. 이 포트는 LAN1 케이블에만 꽂혀 있어서 LAN2 위에는 물리적으로 존재하지 않는다. R 이 LAN2 로 프레임을 내보낼 때는 **LAN2 에 꽂힌 포트(eth1) 의 MAC**, 즉 `22:22:..` 을 src MAC 으로 써야 LAN2 위의 스위치·서버가 그 프레임을 인식한다.

**비유** — 통역사가 한국인 방(LAN1) 과 미국인 방(LAN2) 사이에서 통역한다. 한국인 방에서는 "한국어 입" 으로 말하고, 미국인 방으로 넘어가면 "영어 입" 으로 바꿔 말한다. 전달하는 내용(IP) 은 같지만, 그 방에서 통하는 언어(MAC) 로 바꿔줘야 상대가 듣는다.
